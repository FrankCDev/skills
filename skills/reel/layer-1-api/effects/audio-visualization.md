...
description: Audio visualization patterns including spectrum bars, waveforms, bass-reactive effects, and postprocessing
metadata:
  tags: audio, visualization, spectrum, waveform, bass, music, frequency
---
...
Use `useWindowedAudioData()` to load audio data efficiently:
...
Use `getWaveformPortion()` for simplified volume data:
...
- Calling `useCurrentFrame()` in child components inside a `<Sequence>` -- pass `frame` as a prop from the parent instead.
- Using a `numberOfSamples` value that is not a power of 2 -- this will throw an error.
- Forgetting to check `if (!audioData) return null` -- `audioData` is null while loading.
- Not passing `dataOffsetInSeconds` to `visualizeAudio()` -- this causes misalignment with windowed audio data.
- Omitting `optimizeFor: "speed"` in production -- rendering will be slow for high sample counts.
